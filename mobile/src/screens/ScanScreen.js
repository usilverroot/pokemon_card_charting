...
export default function ScanScreen({ onSelectCard }) {
  ...
  if (photo) {
    ...
    const debugImages = recognitionResult?.debug_images;
    ...
    return (
      <Screen>
        <ScrollView style={styles.resultScroll} contentContainerStyle={styles.resultContent}>
            ...
            <View style={styles.cardsPanel}>
              ...
              {cards.length > 0 ? (
                cards.slice(0, 5).map((card) => (
                  <CardListItem key={card.card_id} card={card} onPress={onSelectCard} />
                ))
              ) : (
          ...
          {debugImages ? (
            <View style={styles.debugPanel}>
              <Pressable style={styles.debugHeader} onPress={() => setShowDebug((prev) => !prev)}>
                <Text style={styles.sectionTitle}>OCR 디버그</Text>
                <Ionicons
              ...
              {showDebug ? (
                <View style={styles.imageSection}>
                  <Text style={styles.subLabel}>보정된 사진</Text>
                  <Image
                    source={{ uri: imageUrl(debugImages.normalized) }}
                    style={styles.normalizedImage}
                    contentFit="contain"
                  />
                  <Text style={styles.subLabel}>OCR crop</Text>
                  <View style={styles.cropGrid}>
                    <DebugImage label="하단 gray" uri={imageUrl(debugImages.ocr_bottom_gray || debugImages.ocr_bottom)} />
                    <DebugImage label="하단 CLAHE" uri={imageUrl(debugImages.ocr_bottom_clahe || debugImages.ocr_bottom)} />
                    <DebugImage label="하단 adaptive" uri={imageUrl(debugImages.ocr_bottom_adaptive || debugImages.ocr_bottom)} />
                    <DebugImage label="하단 inverted" uri={imageUrl(debugImages.ocr_bottom_inverted || debugImages.ocr_bottom)} />
                    <DebugImage label="상단 이름" uri={imageUrl(debugImages.ocr_name)} />
                    <DebugImage label="에너지" uri={imageUrl(debugImages.ocr_energy)} />
                  </View>
                </View>
              ) : null}
            </View>
        ...
        <View style={styles.actionRow}>
          <IconButton icon="refresh" label="다시 촬영" onPress={resetScan} />
